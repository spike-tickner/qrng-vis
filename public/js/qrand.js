...
/**
 * num: number of 8 bit integers to retrieve as a string.
 * fn: called as fn(err, uint) where uint is an array of integer strings.
 *     If error, called as fn(err, null). Otherwise fn(null, ints).
 *     To convert to a single string just call ints.join('') on the result array.
 */
exports.getRandomInt = function requestRandomInt(num, fn) {
  if (num < 1) return fn(new Error('Invalid length (< 1)'));
  http.get(baseInt + num , function (res) {
    var body = '';
    res.on('error', function (err) {
      return fn(new Error(err));
    });
